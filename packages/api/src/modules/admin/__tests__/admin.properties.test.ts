...
describe('Property 36: Business Suspension', () => {
  ...
  it('canSuspend returns true only for exactly "active"', () => {
    fc.assert(
      fc.property(
        fc.constantFrom('active', 'suspended', 'pending_verification', 'inactive', '', 'ACTIVE', 'Active'),
        (status) => {
...
describe('Property 37: Business Reactivation', () => {
  ...
  it('canReactivate returns false for any status other than suspended', () => {
    fc.assert(
      fc.property(
        fc.string().filter((s) => s !== 'suspended'),
        (status) => {
  ...
  it('canReactivate returns true only for exactly "suspended"', () => {
    fc.assert(
      fc.property(
        fc.constantFrom('suspended', 'active', 'pending_verification', 'inactive', '', 'SUSPENDED', 'Suspended'),
        (status) => {
