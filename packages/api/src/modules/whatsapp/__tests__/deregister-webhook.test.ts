...
import { deregisterWebhook } from '../whatsapp-integration.service.js';
...
describe('deregisterWebhook', () => {
  it('returns success and sets status to inactive when Meta API responds 200', async () => {
    const mockQuery = vi.mocked(pool.query);

    // First call: getCredentials SELECT
    mockQuery.mockResolvedValueOnce({ rows: [makeIntegrationRow()], rowCount: 1 } as never);
    // Second call: UPDATE to inactive
    ...
    const result = await deregisterWebhook('biz-1');

    expect(result.success).toBe(true);
    expect(result.errorMessage).toBeUndefined();

    // Verify fetch was called with DELETE method and correct URL
    ...
    // Verify DB was updated to inactive with registered_at cleared
    const updateCall = mockQuery.mock.calls[1];
    expect(updateCall[0]).toMatch(/status = 'inactive'/);
    expect(updateCall[0]).toMatch(/registered_at = NULL/);
  ...
  it('returns failure and sets status to error (retaining credentials) when Meta API returns non-200', async () => {
    ...
    // Confirm no DELETE was issued against the integrations table
  ...
  it('returns failure and sets status to error when fetch throws a network error', async () => {
    ...
    // Credentials retained — status set to error, no DELETE
    const updateCall = mockQuery.mock.calls[1];
    expect(updateCall[0]).toMatch(/status = 'error'/);
  ...
  it('returns descriptive error without DB writes when no integration exists', async () => {
    ...
    const result = await deregisterWebhook('biz-unknown');
