...
function makeIntegrationRow(wabaId: string, phoneNumberId: string, accessToken: string) {
  return {
    id: 'int-prop-1',
...
describe('Property 11: Failed Webhook Verification Retains Credentials', () => {
      ...
      fc.asyncProperty(
        ...
        async (wabaId, phoneNumberId, accessToken, httpStatus) => {
          ...
          vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(
            new Response(JSON.stringify({ error: { message: `HTTP ${httpStatus} error` } }), {
              status: httpStatus,
            }),
          );
          ...
          const updateCall = mockQuery.mock.calls.find((c) =>
            (c[0] as string).includes("status = 'error'"),
          );
          ...
          const errorMsgInDb = updateCall![1][0] as string;
  ...
  it('retains credentials and returns error for any combination of credentials and error scenario', async () => {
    const errorScenarioArb = fc.oneof(
      ...
      // Network error
      networkErrorArb.map((msg) => ({ type: 'network' as const, message: msg })),
    );
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        nonEmptyStringArb,
        errorScenarioArb,
        async (wabaId, phoneNumberId, accessToken, scenario) => {
          const mockQuery = vi.mocked(pool.query);
          mockQuery.mockReset();
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (businessId, wabaId, phoneNumberId, accessToken, webhookVerifyToken) => {
          ...
          const allQueries: string[] = mockQuery.mock.calls.map((c) => (c[0] as string).toLowerCase());
...
describe('Property 20: Media Size Fallback', () => {
  ...
  it('uses text fallback and never calls Meta API for any file size > 16 MB', async () => {
    await fc.assert(
      fc.asyncProperty(
        mediaMsgArb,
        nonEmptyStringArb, // textFallback
        ...
        async (mediaMsg, textFallback, fileSizeBytes) => {
          ...
          const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(
            new Response(JSON.stringify({ messages: [{ id: 'msg-p20' }] }), { status: 200 }),
          );
          ...
          const textPayload = sentBody.text as { body: string };
