...
describe('Property 13: Claude Haiku Is the Only Inference Model', () => {
  ...
  it('model parameter sent to Claude API always equals CLAUDE_HAIKU_MODEL', async () => {
    await fc.assert(fc.asyncProperty(strArb, strArb, async (sp, um) => {
      ...
      expect(body).not.toBeNull(); expect(body!['model']).toBe(CLAUDE_HAIKU_MODEL); expect((body!['model'] as string).toLowerCase()).toContain('haiku');
    }), { numRuns: 25 });
  });
  it('model is never sonnet, opus, or gpt', async () => {
    await fc.assert(fc.asyncProperty(strArb, strArb, async (sp, um) => {
      ...
      const { callClaudeHaiku } = await import('../conversation-engine.service.js');
      await callClaudeHaiku(sp, [], um);
      expect(m).not.toBeNull(); expect(m!.toLowerCase()).not.toContain('sonnet'); expect(m!.toLowerCase()).not.toContain('opus'); expect(m!.toLowerCase()).not.toContain('gpt'); expect(m!.toLowerCase()).toContain('haiku');
    }), { numRuns: 25 });
  });
});
...
describe('Property 16: Deactivating Manual Intervention Restores AI Responses', () => {
  ...
  it('processInboundMessage proceeds to AI when manual_intervention_active = false', async () => {
    await fc.assert(fc.asyncProperty(uuidArb, strArb, strArb, async (bid, cwa, mt) => {
      ...
      mq.mockResolvedValueOnce({ rows: [], rowCount: 0 } as never);
      mq.mockResolvedValueOnce({ rows: [{ context_summary: null }], rowCount: 1 } as never);
      ...
      mq.mockResolvedValueOnce({ rows: [], rowCount: 1 } as never); mq.mockResolvedValueOnce({ rows: [], rowCount: 1 } as never); mq.mockResolvedValueOnce({ rows: [], rowCount: 1 } as never);
