...
describe('Property 30: WhatsApp Number Masking', () => {
  it('maskWaNumber always starts with ****', () => {
    fc.assert(
      fc.property(waNumberArb, (waNumber) => {
  ...
  it('masked number has exactly 8 characters when input is exactly 4 chars', () => {
    fc.assert(
      fc.property(
        fc.string({ minLength: 4, maxLength: 4 }).filter((s) => !s.includes('\0')),
        (waNumber) => {
          const masked = maskWaNumber(waNumber);
          // ****{4chars} = 8 chars
 ...
 */
function computeRevenueSummary(amounts: number[]): {
  totalRevenue: number;
  totalOrders: number;
...
describe('Property 31: Revenue Summary Correctness', () => {
  it('averageOrderValue = totalRevenue / totalOrders when totalOrders > 0', () => {
    fc.assert(
      fc.property(
        fc.array(fc.float({ min: Math.fround(0.01), max: Math.fround(10000), noNaN: true }), { minLength: 1, maxLength: 50 }),
        (amounts) => {
          const { totalRevenue, totalOrders, averageOrderValue } =
            computeRevenueSummary(amounts);
  ...
  it('averageOrderValue = 0 when totalOrders = 0', () => {
    fc.assert(
      fc.property(fc.constant([]), (amounts: number[]) => {
  ...
  it('totalRevenue is always >= 0 for non-negative order amounts', () => {
    fc.assert(
      fc.property(
        fc.array(fc.float({ min: Math.fround(0), max: Math.fround(10000), noNaN: true }), { minLength: 0, maxLength: 50 }),
        (amounts) => {
  ...
  it('totalOrders equals the number of orders in the input', () => {
    fc.assert(
      fc.property(
        fc.array(fc.float({ min: Math.fround(0.01), max: Math.fround(10000), noNaN: true }), { minLength: 0, maxLength: 50 }),
        (amounts) => {
    ...
    fc.assert(
      fc.property(
        ...
        (orders) => {
          ...
          expect(lines[0]).toBe(
            'Order Reference,Customer (masked),Status,Total Amount,Currency,Date',
          );
        },
      ),
      { numRuns: 25 },
  ...
  it('filtering tickets by businessId returns only tickets for that business', () => {
    fc.assert(
      fc.property(
        ...
        (targetBusinessId, otherBusinessId, allTickets) => {
