...
describe('Property 25: Out-of-Stock Products Excluded from Responses', () => {
  ...
  it('getInStockProducts returns empty array when all products are out of stock', async () => {
    await fc.assert(
      fc.asyncProperty(uuidArb, async (businessId) => {
  ...
  it('a product with stock_quantity=0 is never present in getInStockProducts result', async () => {
    await fc.assert(
      fc.asyncProperty(
        uuidArb,
        fc.array(
          fc.record({ id: uuidArb, stock_quantity: fc.integer({ min: 1, max: 100 }), name: nameArb }),
          { minLength: 1, maxLength: 10 },
        ),
        async (businessId, inStockRows) => {
          const rows = inStockRows.map((r) =>
            makeProductRow({ id: r.id, business_id: businessId, stock_quantity: r.stock_quantity, name: r.name }),
          );
...
describe('Property 26: Product Filter Correctness', () => {
  ...
  it('every product in result satisfies the inStock=false filter', async () => {
    await fc.assert(
      fc.asyncProperty(
        uuidArb,
        fc.array(
          fc.record({ id: uuidArb, stock_quantity: fc.constant(0), name: nameArb }),
          { minLength: 0, maxLength: 15 },
        ),
        async (businessId, outOfStockRows) => {
          const rows = outOfStockRows.map((r) =>
            makeProductRow({ id: r.id, business_id: businessId, stock_quantity: 0, name: r.name }),
          );
          vi.mocked(pool.query).mockResolvedValueOnce({ rows, rowCount: rows.length } as never);
  ...
  it('every product in result satisfies the minPrice filter', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        async (businessId, minPrice, priceRows) => {
          ...
          vi.mocked(pool.query).mockResolvedValueOnce({ rows: filteredRows, rowCount: filteredRows.length } as never);
  ...
  it('every product in result satisfies the maxPrice filter', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        async (businessId, maxPrice, priceRows) => {
          const filteredRows = priceRows
  ...
  it('no matching product is absent: result count equals DB row count', async () => {
    await fc.assert(
      fc.asyncProperty(
        uuidArb,
        fc.array(fc.record({ id: uuidArb, name: nameArb }), { minLength: 0, maxLength: 20 }),
        async (businessId, productRows) => {
          const rows = productRows.map((r) =>
            makeProductRow({ id: r.id, business_id: businessId, name: r.name }),
          );
          vi.mocked(pool.query).mockResolvedValueOnce({ rows, rowCount: rows.length } as never);
...
describe('Property 27: Active Combo Presented as Single Carousel Item', () => {
  ...
  it('carousel item price is always the promo_price (never sum of individual prices)', async () => {
    ...
    vi.mocked(pool.query).mockResolvedValueOnce({ rows: [comboRow], rowCount: 1 } as never);
  ...
  it('isCombo flag is always true for combo carousel items', async () => {
    await fc.assert(
      fc.asyncProperty(
        uuidArb,
        fc.array(
          fc.record({ id: uuidArb, name: nameArb, promo_price: fc.constant('9.99'), currency: currencyArb }),
...
describe('Property 28: CSV Import Error Reporting', () => {
  it('each skipped row includes a row number and a reason', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        async (businessId, rowDefs) => {
          const header = 'name,description,price,currency,stock_quantity,category';
          const dataLines = rowDefs.map(
            (r) => `${r.name},${r.description},${r.price},${r.currency},${r.stock_quantity},${r.category}`,
          );
  ...
  it('validateCsvRow returns a reason for any row missing a required field', () => {
    const requiredFields = ['name', 'price', 'currency', 'stock_quantity'] as const;
    ...
    fc.assert(
      fc.property(
        ...
        currencyArb,
        fc.integer({ min: 0, max: 1000 }).map(String),
        (missingField, name, price, currency, stock_quantity) => {
          const row: Record<string, string> = { name, price, currency, stock_quantity };
