...
import type { OrderItem } from '../payment.service.js';
  ...
  it('receipt contains order reference', () => {
    fc.assert(
      fc.property(
        ...
        (orderReference, items, totalAmount, currency, timestamp) => {
    ...
    fc.assert(
      fc.property(
        ...
        (orderReference, items, totalAmount, currency, timestamp) => {
          ...
          expect(body).toContain(orderReference);
          // Field 2: item names
          for (const item of items) expect(body).toContain(item.productName);
...
function isOrderStale(expiresAt: Date, now: Date): boolean {
...
function isWithdrawalValid(amountUsd: number, availableBalance: number): boolean {
...
describe('Property 23: Transaction Record Has All Five Required Fields', () => {
  ...
  it('order object always has all 5 required fields non-null', () => {
    fc.assert(
      fc.property(
        fc.constantFrom('pending', 'completed', 'expired', 'failed'),
        fc.integer({ min: 1, max: 100000 }).map((n) => n / 100),
        fc.constantFrom('USD', 'ZWL', 'EUR'),
        fc.uuid(),
        (status, amount, currency, businessId) => {
          const ref = generateOrderReference();
...
describe('Property 39: Auto-Processing Threshold', () => {
  it('shouldAutoProcess returns true for any amount strictly below threshold', () => {
    ...
    fc.assert(
      fc.property(
        fc.integer({ min: 1, max: Math.floor(threshold * 100) - 1 }),
        (amountCents) => {
  ...
  it('shouldAutoProcess returns false for any amount >= threshold', () => {
    ...
    fc.assert(
      fc.property(
        fc.integer({ min: Math.ceil(threshold * 100), max: Math.ceil(threshold * 100) * 10 }),
        (amountCents) => {
          const amount = amountCents / 100;
          fc.pre(amount >= threshold);
...
const externalDetailsArb: fc.Arbitrary<Record<string, string>> = fc
  .record({
...
const invoiceMessageArb: fc.Arbitrary<InvoiceMessage> = fc.record({
...
describe('Property 42: Disabling In_Chat_Payments Requires External_Payment_Details', () => {
  ...
  it('isExternalDetailsValid returns true when at least one field is non-empty', () => {
    fc.assert(
      fc.property(externalDetailsArb, (details) => {
  ...
  it('disabling with no details is rejected (isExternalDetailsValid returns false)', () => {
    fc.assert(
      fc.property(
        ...
        (details) => {
          expect(isExternalDetailsValid(details as Record<string, string> | null | undefined)).toBe(false);
        },
      ),
      { numRuns: 25 },
    );
...
describe('Property 44: Payment Settings Round-Trip', () => {
  it('round-trip: settings stored and retrieved are identical', () => {
    fc.assert(
      fc.property(
        fc.boolean(),
        fc.oneof(fc.constant(null), externalDetailsArb),
        (enabled, details) => {
...
describe('Property 41: Invoice Content Completeness When In_Chat_Payments Disabled', () => {
  it('invoice always contains order reference', () => {
    fc.assert(
      fc.property(invoiceMessageArb, (invoice) => {
  ...
  it('invoice always contains at least one external payment detail entry', () => {
    fc.assert(
      fc.property(invoiceMessageArb, (invoice) => {
        const msg = buildInvoiceMessage(invoice);
        const hasDetail = Object.entries(invoice.externalPaymentDetails).some(
          ([, v]) => v.trim() !== '' && msg.includes(v),
        );
...
describe('Property 43: Toggle Change Applies Immediately to Subsequent Orders', () => {
  it('after toggle to disabled, next order uses invoice flow (paynowLink=null)', () => {
    fc.assert(
      fc.property(fc.constant(false), (newEnabled) => {
  ...
  it('after toggle to enabled, next order uses Paynow flow (paynowLink non-null)', () => {
    fc.assert(
      fc.property(fc.constant(true), (newEnabled) => {
        const flow = determineOrderFlow(newEnabled);
        expect(flow.paynowLink).not.toBeNull();
        expect(flow.usePaynow).toBe(true);
      }),
