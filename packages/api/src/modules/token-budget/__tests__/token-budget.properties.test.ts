...
function computeBudgetStatus(
  ...
  capUsd: number,
): { allowed: boolean; remainingUsd: number; suspended: boolean } {
...
function computeAlertState(
  accumulatedCostUsd: number,
  capUsd: number,
  alert80SentBefore: boolean,
  alert95SentBefore: boolean,
): { alert80Sent: boolean; alert95Sent: boolean; suspended: boolean } {
...
describe('Property 8: Tier Cost Cap Enforcement', () => {
  it('once accumulated cost >= cap, allowed is always false', () => {
    ...
    fc.assert(
      fc.property(tierArb, fc.float({ min: 0, max: 500, noNaN: true }), (tier, accumulated) => {
  ...
  it('suspension persists: any additional cost increment while suspended keeps allowed=false', () => {
    fc.assert(
      fc.property(
        ...
        (initialAccumulated, cap, additionalCostCents) => {
