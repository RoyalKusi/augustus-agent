...
function makeConversation(
  ...
  interventionStart: Date | null = null,
  interventionEnd: Date | null = null,
) {
  return {
    id: 'conv-test',
  ...
  it('manual_intervention_active = false is the only condition needed to allow AI', () => {
    fc.assert(
      fc.property(
        fc.boolean(),
        uuidArb,
        (hadPriorIntervention, agentId) => {
          // Regardless of whether there was a prior intervention session,
          // if the flag is now false, AI is allowed
          const conv = makeConversation(
            false,
            hadPriorIntervention ? agentId : null,
            hadPriorIntervention ? new Date(Date.now() - 60000) : null,
            hadPriorIntervention ? new Date() : null,
          );
          expect(isManualInterventionActive(conv)).toBe(false);
        },
      ),
    ...
    fc.assert(
      fc.property(
        ...
        (agentId, durationMs) => {
          ...
          expect(log.interventionEnd!.getTime()).toBeGreaterThan(
            log.interventionStart!.getTime(),
          );
        },
      ),
      { numRuns: 25 },
