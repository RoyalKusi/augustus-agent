...
import { activateSubscription, upgradePlan, downgradePlan, getActiveSubscription, } from './subscription.service.js';
import { handleSubscriptionPaymentWebhook, initiateSubscriptionCharge, pollSubscriptionPaymentStatus, } from './paynow.subscription.js';
...
export async function subscriptionRoutes(app) {
    ...
    app.post('/subscription/activate', { preHandler: authenticate }, async (request, reply) => {
        const businessId = request.businessId;
        const { tier, paynowReference } = request.body;
        if (!isValidTier(tier)) {
    ...
    app.post('/subscription/upgrade', async (request, reply) => {
        const { businessId, newTier, paynowReference } = request.body;
    ...
    app.post('/subscription/downgrade', async (request, reply) => {
        const { businessId, newTier } = request.body;
        if (!isValidTier(newTier)) {
            return reply.status(400).send({ error: 'Invalid tier.' });
        }
        try {
    ...
    app.post('/subscription/initiate-payment', { preHandler: authenticate }, async (request, reply) => {
        ...
        try {
            const result = await initiateSubscriptionCharge(businessId, email, plan.priceUsd, `Augustus ${plan.displayName} subscription`, tier);
            if (!result.success) {
    ...
    app.post('/subscription/poll-payment', { preHandler: authenticate }, async (request, reply) => {
        const businessId = request.businessId;
        const { paynowReference, pollUrl, tier } = request.body;
        ...
        if (!resolvedPollUrl) {
            const stored = await pool.query(`SELECT poll_url FROM subscription_payments WHERE paynow_reference = $1 AND business_id = $2 LIMIT 1`, [paynowReference, businessId]);
        ...
        try {
            ...
            if (result.status === 'paid') {
                ...
                await pool.query(`UPDATE subscription_payments SET status = 'paid', updated_at = NOW() WHERE paynow_reference = $1`, [result.paynowReference ?? paynowReference]);
