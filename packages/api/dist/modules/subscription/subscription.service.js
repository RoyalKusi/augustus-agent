...
import { getPlan, calculateProration, isValidTier } from './plans.js';
import { sendSubscriptionRenewalReminder, sendSubscriptionSuspendedEmail, sendPaymentFailedEmail, } from '../../services/notification.stub.js';
 ...
 */
export async function activateSubscription(businessId, tier, paynowReference) {
    if (!isValidTier(tier)) {
    ...
    try {
        ...
        await client.query(`UPDATE subscriptions SET status = 'cancelled', updated_at = NOW()
       WHERE business_id = $1 AND status = 'active'`, [businessId]);
        const result = await client.query(`INSERT INTO subscriptions
         (business_id, plan, price_usd, status, activation_timestamp, renewal_date,
          billing_cycle_start, paynow_reference)
       VALUES ($1, $2, $3, 'active', $4, $5, $6, $7)
       RETURNING *`, [businessId, tier, plan.priceUsd, now, renewalDate, cycleStart, paynowReference]);
        // Ensure business status is active
        await client.query(`UPDATE businesses SET status = 'active', updated_at = NOW() WHERE id = $1`, [businessId]);
        // Initialise token_usage record for this billing cycle
        await client.query(`INSERT INTO token_usage (business_id, billing_cycle_start)
       VALUES ($1, $2)
       ON CONFLICT (business_id, billing_cycle_start) DO NOTHING`, [businessId, cycleStart.toISOString().split('T')[0]]);
 ...
 */
export async function upgradePlan(businessId, newTier, paynowReference) {
    ...
    const proratedChargeUsd = calculateProration(currentPlan.priceUsd, newPlan.priceUsd, cycleStart, cycleEnd, now);
    const result = await pool.query(`UPDATE subscriptions
     SET plan = $1, price_usd = $2, paynow_reference = $3, updated_at = NOW()
     WHERE id = $4
     RETURNING *`, [newTier, newPlan.priceUsd, paynowReference, current.id]);
 ...
 */
export async function downgradePlan(businessId, newTier) {
    const current = await getActiveSubscription(businessId);
    if (!current)
        throw new Error('No active subscription found.');
    const currentPlan = getPlan(current.plan);
...
export async function sendRenewalReminders() {
    const result = await pool.query(`SELECT s.id, s.business_id, s.plan, s.renewal_date,
            b.email,
    ...
    for (const row of result.rows) {
        ...
        const daysUntilRenewal = Math.round((renewalDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
        if (daysUntilRenewal === 7 && !row.reminder_7_sent) {
            await sendSubscriptionRenewalReminder(row.email, renewalDate, 7);
            await pool.query(`UPDATE subscriptions SET reminder_7_sent = TRUE, updated_at = NOW() WHERE id = $1`, [row.id]);
        }
        else if (daysUntilRenewal === 1 && !row.reminder_1_sent) {
            await sendSubscriptionRenewalReminder(row.email, renewalDate, 1);
            await pool.query(`UPDATE subscriptions SET reminder_1_sent = TRUE, updated_at = NOW() WHERE id = $1`, [row.id]);
        }
    }
}
 ...
 */
export async function handleFailedRenewalPayment(subscriptionId) {
    const result = await pool.query(`SELECT s.id, s.business_id, s.plan, s.failed_payment_attempts, b.email
     FROM subscriptions s
     JOIN businesses b ON b.id = s.business_id
    ...
    if (attempts >= 2) {
        ...
        try {
         ...
         WHERE id = $2`, [attempts, subscriptionId]);
            await client.query(`UPDATE businesses SET status = 'suspended', updated_at = NOW() WHERE id = $1`, [sub.business_id]);
            await client.query('COMMIT');
        }
        catch (err) {
            await client.query('ROLLBACK');
       ...
       WHERE id = $3`, [attempts, retryAt, subscriptionId]);
        await sendPaymentFailedEmail(sub.email, retryAt);
    }
}
// ─── Helpers ──────────────────────────────────────────────────────────────────
export async function getActiveSubscription(businessId) {
    const result = await pool.query(`SELECT * FROM subscriptions WHERE business_id = $1 AND status = 'active' LIMIT 1`, [businessId]);
