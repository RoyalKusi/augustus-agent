...
import { sendBudgetAlert80Email, sendBudgetAlert95Email, } from '../../services/notification.stub.js';
 ...
 */
export async function checkBudget(businessId) {
 ...
 */
export async function recordInferenceCost(businessId, costUsd, businessEmail) {
    const cycleStart = await getCurrentCycleStart(businessId);
    // Upsert usage row and atomically increment cost
    const result = await pool.query(`INSERT INTO token_usage (business_id, billing_cycle_start, accumulated_cost_usd)
     VALUES ($1, $2, $3)
     ON CONFLICT (business_id, billing_cycle_start)
     DO UPDATE SET
       accumulated_cost_usd = token_usage.accumulated_cost_usd + $3,
       updated_at = NOW()
     RETURNING *`, [businessId, cycleStart, costUsd]);
 ...
 */
export async function setHardLimitOverride(businessId, hardLimitUsd, operatorId) {
    await pool.query(`INSERT INTO business_token_overrides (business_id, hard_limit_usd, set_by_operator_id)
     VALUES ($1, $2, $3)
     ON CONFLICT (business_id)
     DO UPDATE SET hard_limit_usd = $2, set_by_operator_id = $3, updated_at = NOW()`, [businessId, hardLimitUsd, operatorId]);
}
/**
 * Remove a hard limit override, reverting to tier default.
 */
export async function removeHardLimitOverride(businessId) {
    await pool.query(`DELETE FROM business_token_overrides WHERE business_id = $1`, [businessId]);
}
...
export async function resetBillingCycle(businessId) {
    ...
    await pool.query(`INSERT INTO token_usage (business_id, billing_cycle_start, accumulated_cost_usd,
       alert_80_sent, alert_95_sent, suspended)
     VALUES ($1, $2, 0, FALSE, FALSE, FALSE)
     ON CONFLICT (business_id, billing_cycle_start) DO NOTHING`, [businessId, newCycleStart]);
}
/**
 * Scheduled job: reset all businesses whose billing cycle starts today.
 * Intended to be called by a daily cron job.
 */
export async function runBillingCycleResetJob() {
    const today = new Date().toISOString().split('T')[0];
    // Find all active subscriptions whose billing cycle starts today
    const result = await pool.query(`SELECT s.business_id
     FROM subscriptions s
     WHERE s.status = 'active'
       AND TO_CHAR(s.billing_cycle_start, 'MM-DD') = TO_CHAR($1::date, 'MM-DD')`, [today]);
 ...
 */
export async function shouldSendUnavailabilityMessage(businessId) {
    const cycleStart = await getCurrentCycleStart(businessId);
    // Use a dedicated column to track if the unavailability message was sent this suspension
    const result = await pool.query(`SELECT unavailability_msg_sent
     FROM token_usage
     WHERE business_id = $1 AND billing_cycle_start = $2`, [businessId, cycleStart]);
    ...
    await pool.query(`UPDATE token_usage
     ...
     WHERE business_id = $1 AND billing_cycle_start = $2
       AND unavailability_msg_sent = FALSE`, [businessId, cycleStart]);
    return true;
}
// ─── Helpers ──────────────────────────────────────────────────────────────────
/**
 ...
 */
async function getCurrentCycleStart(businessId) {
    const result = await pool.query(`SELECT TO_CHAR(billing_cycle_start, 'YYYY-MM-DD') AS billing_cycle_start
     FROM subscriptions
 ...
 */
async function getUsageAndCap(businessId) {
    const cycleStart = await getCurrentCycleStart(businessId);
    const [usageResult, overrideResult, subResult] = await Promise.all([
        pool.query(`SELECT * FROM token_usage
       WHERE business_id = $1 AND billing_cycle_start = $2`, [businessId, cycleStart]),
        pool.query(`SELECT hard_limit_usd FROM business_token_overrides WHERE business_id = $1`, [businessId]),
        pool.query(`SELECT plan FROM subscriptions WHERE business_id = $1 AND status = 'active' LIMIT 1`, [businessId]),
    ]);
