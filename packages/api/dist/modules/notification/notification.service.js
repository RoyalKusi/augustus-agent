...
async function sendViaSendGrid(to, subject, htmlBody, textBody, apiKey, fromAddress, fromName) {
    const content = [
...
async function sendViaSes(to, subject, htmlBody, textBody, apiKey, fromAddress, fromName) {
    ...
    console.warn(`[Notification] SES fetch fallback: AWS SDK unavailable. Email to ${to} not sent.`);
    void apiKey; // suppress unused warning
}
// ─── Task 14.2: Email templates ───────────────────────────────────────────────
export const emailTemplates = {
    registrationVerification(verificationUrl) {
        return {
    ...
    subscriptionReminder(planName, daysUntilRenewal, renewalDate) {
        return {
    ...
    budgetAlert(usagePercent, planName) {
        return {
    ...
    accountSuspension(businessName) {
        return {
    ...
    supportTicketAck(ticketReference, subject) {
        return {
    ...
    supportTicketStatusUpdate(ticketReference, newStatus) {
...
export async function sendSupportTicketAck(to, ticketReference, subject) {
...
export async function sendSupportTicketStatusUpdate(to, ticketReference, newStatus) {
