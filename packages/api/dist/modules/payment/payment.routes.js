...
import { generatePaynowLink, handlePaynowWebhook, pollPaynowStatus, expireStaleOrders, createWithdrawalRequest, processWithdrawal, getRevenueBalance, getOrderWithItems, getPaymentSettings, updatePaymentSettings, } from './payment.service.js';
export async function paymentRoutes(app) {
    ...
    app.post('/payments/initiate', { preHandler: authenticate }, async (request, reply) => {
        ...
        try {
            const { order, paymentUrl } = await generatePaynowLink(businessId, body.customer_wa_number, items, body.currency, body.conversation_id);
    ...
    app.post('/payments/withdrawals', { preHandler: authenticate }, async (request, reply) => {
        ...
        try {
            const { withdrawal, autoProcessed } = await createWithdrawalRequest(businessId, body.amount_usd, body.paynow_merchant_ref);
            return reply.status(201).send({ withdrawal, autoProcessed });
        }
        catch (err) {
            if (err instanceof Error && err.statusCode === 422) {
                const typedErr = err;
