 ...
 */
export async function generatePaynowLink(businessId, customerWaNumber, items, currency, conversationId) {
    const totalAmount = items.reduce((sum, i) => sum + i.unitPrice * i.quantity, 0);
    const orderReference = generateOrderReference();
    // Call Paynow API to create payment link
    const paynowResult = await initiatePaynowPayment(orderReference, customerWaNumber, totalAmount, currency, items.map((i) => `${i.productName} x${i.quantity}`).join(', '));
    ...
    try {
       ...
       RETURNING id, business_id, conversation_id, customer_wa_number, order_reference,
        ...
        for (const item of items) {
         ...
         VALUES ($1, $2, $3, $4)`, [orderRow.id, item.productId, item.quantity, item.unitPrice]);
        }
 ...
 */
export async function initiatePaynowPayment(orderReference, email, amount, _currency, description) {
 ...
 */
async function validatePaynowWebhookHash(payload) {
    ...
    const hashInput = Object.entries(payload)
 ...
 */
export async function handlePaynowWebhook(payload) {
    ...
    if (!reference)
        return;
    const orderResult = await pool.query(`SELECT id, business_id, customer_wa_number, payment_status AS status FROM orders WHERE order_reference = $1`, [reference]);
    ...
    if (status === 'paid') {
        ...
        await pool.query(`UPDATE orders SET payment_status = 'failed', paynow_reference = $1 WHERE id = $2`, [paynowReference, order.id]);
    }
}
/**
 ...
 */
export async function pollPaynowStatus(orderId) {
    const orderResult = await pool.query(`SELECT id, business_id, paynow_reference, paynow_poll_url, payment_status AS status, order_reference FROM orders WHERE id = $1`, [orderId]);
    if (orderResult.rows.length === 0)
        return;
    const order = orderResult.rows[0];
    if (order.status !== 'pending')
    ...
    const pollUrl = order.paynow_poll_url ??
 ...
 */
export async function dispatchReceipt(businessId, customerWaNumber, orderReference, items, totalAmount, currency, timestamp) {
 ...
 */
export async function expireOrderIfStale(orderId) {
    const result = await pool.query(`UPDATE orders
     SET payment_status = 'expired', updated_at = NOW()
     WHERE id = $1
       ...
       AND payment_link_expires_at IS NOT NULL
       AND payment_link_expires_at < NOW()
     RETURNING id, business_id, customer_wa_number, order_reference`, [orderId]);
 ...
 */
export async function confirmPayment(orderId, paynowReference) {
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        // Mark order completed
        const orderResult = await client.query(`UPDATE orders
       SET payment_status = 'completed', paynow_reference = $1, completed_at = NOW(), updated_at = NOW()
       WHERE id = $2 AND payment_status = 'pending'
       RETURNING id, business_id, customer_wa_number, order_reference, total_amount, currency`, [paynowReference, orderId]);
        if (orderResult.rows.length === 0) {
            await client.query('ROLLBACK');
            return;
        }
        ...
        // Fetch order items
        const itemsResult = await client.query(`SELECT product_id, quantity, unit_price FROM order_items WHERE order_id = $1`, [orderId]);
        // Task 9.6 — Property 24: decrement stock for each item
        for (const item of itemsResult.rows) {
            await client.query(`UPDATE products
         SET stock_quantity = stock_quantity - $1, updated_at = NOW()
         WHERE id = $2 AND business_id = $3`, [item.quantity, item.product_id, order.business_id]);
        }
        // Task 9.7: upsert revenue balance — update both column sets for compatibility
        await client.query(`INSERT INTO revenue_balances (business_id, available_balance, total_lifetime_revenue, available_usd, lifetime_usd, updated_at)
       ...
       ON CONFLICT (business_id) DO UPDATE
         SET available_balance      = revenue_balances.available_balance + $2,
             ...
             updated_at             = NOW()`, [order.business_id, totalAmount]);
        await client.query('COMMIT');
        // Fetch product names for receipt
        const productIds = itemsResult.rows.map((i) => i.product_id);
        const productsResult = await pool.query(`SELECT id, name FROM products WHERE id = ANY($1)`, [productIds]);
        const nameMap = new Map(productsResult.rows.map((r) => [r.id, r.name]));
        const receiptItems = itemsResult.rows.map((i) => ({
        ...
        await dispatchReceipt(order.business_id, order.customer_wa_number, order.order_reference, receiptItems, totalAmount, order.currency, new Date());
    }
    catch (err) {
        await client.query('ROLLBACK');
        throw err;
...
export async function getRevenueBalance(businessId) {
    const result = await pool.query(`SELECT business_id, available_balance, total_lifetime_revenue, updated_at
     FROM revenue_balances WHERE business_id = $1`, [businessId]);
    if (result.rows.length === 0)
        return null;
    const row = result.rows[0];
    return {
 ...
 */
export async function createWithdrawalRequest(businessId, amountUsd, paynowMerchantRef) {
    ...
    const result = await pool.query(`INSERT INTO withdrawal_requests (business_id, amount_usd, amount, currency, status, paynow_merchant_ref)
     VALUES ($1, $2, $2, 'USD', 'pending', $3)
     RETURNING *`, [businessId, amountUsd, paynowMerchantRef]);
    const row = result.rows[0];
    const withdrawal = {
        id: row.id,
        businessId: row.business_id,
 ...
 */
export async function processWithdrawal(withdrawalId, approvedBy) {
    // Fetch withdrawal
    const wResult = await pool.query(`SELECT id, business_id, amount_usd, paynow_merchant_ref, status
     FROM withdrawal_requests WHERE id = $1`, [withdrawalId]);
    ...
    const payoutRef = await initiatePaynowPayout(w.business_id, Number(w.amount_usd), w.paynow_merchant_ref ?? '');
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        // Deduct from available balance — update both column sets
        await client.query(`UPDATE revenue_balances
       SET available_balance = available_balance - $1,
           available_usd     = available_usd - $1,
           updated_at        = NOW()
       WHERE business_id = $2`, [Number(w.amount_usd), w.business_id]);
        // Mark withdrawal processed
        const updated = await client.query(`UPDATE withdrawal_requests
       SET status = 'processed', paynow_payout_ref = $1, processed_at = NOW(), approved_by = $2
       WHERE id = $3
       RETURNING *`, [payoutRef, approvedBy, withdrawalId]);
        await client.query('COMMIT');
 ...
 */
async function initiatePaynowPayout(businessId, amountUsd, merchantRef) {
 ...
 */
export function getAutoWithdrawalThreshold() {
 ...
 */
export function shouldAutoProcess(amountUsd) {
    return amountUsd < getAutoWithdrawalThreshold();
}
// ─── Helpers: get order with items ───────────────────────────────────────────
export async function getOrderWithItems(orderId) {
    const orderResult = await pool.query(`SELECT *, payment_status AS status, payment_link AS paynow_link,
             payment_link_expires_at AS expires_at
     FROM orders WHERE id = $1`, [orderId]);
    if (orderResult.rows.length === 0)
        return null;
    const row = orderResult.rows[0];
    const itemsResult = await pool.query(`SELECT oi.product_id, oi.quantity, oi.unit_price, p.name AS product_name
     FROM order_items oi
 ...
 */
export function isExternalDetailsValid(details) {
    if (!details)
        return false;
...
export async function getPaymentSettings(businessId) {
    const result = await pool.query(`SELECT in_chat_payments_enabled, external_payment_details
...
export async function updatePaymentSettings(businessId, settings) {
    if (!settings.inChatPaymentsEnabled) {
        if (!isExternalDetailsValid(settings.externalPaymentDetails)) {
            const err = new Error('External payment details required when in-chat payments are disabled.');
            err.statusCode = 422;
            throw err;
        }
    }
    await pool.query(`UPDATE businesses
     SET in_chat_payments_enabled = $1, external_payment_details = $2
     WHERE id = $3`, [settings.inChatPaymentsEnabled, settings.externalPaymentDetails, businessId]);
    return settings;
}
 ...
 */
export function buildPaymentSettingsResponse(enabled, details) {
...
export function buildInvoiceMessage(invoice) {
    ...
    let paymentLines;
    const details = invoice.externalPaymentDetails;
    const methods = details?.methods;
 ...
 */
export function determineOrderFlow(inChatPaymentsEnabled) {
