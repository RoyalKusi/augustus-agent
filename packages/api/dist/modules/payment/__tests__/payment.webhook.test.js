 ...
 */
function buildValidPaynowPayload(fields, integrationKey) {
    const hashInput = Object.entries(fields)
        .map(([, value]) => value)
        .join('') + integrationKey;
    const hash = computeMd5(hashInput).toUpperCase();
    return { ...fields, hash };
}
function validatePaynowHash(payload, integrationKey) {
...
describe('Task 9.2: Webhook hash validation — property tests', () => {
    ...
    it('any payload with a modified field value after signing is rejected', () => {
        ...
        }), fc.string({ minLength: 1, maxLength: 20 }), (fields, tamperValue) => {
            const payload = buildValidPaynowPayload(fields, INTEGRATION_KEY);
...
describe('Task 9.2: Poll URL selection', () => {
     ...
     */
    function selectPollUrl(paynowPollUrl, paynowReference) {
    ...
    it('property: stored poll URL always takes precedence over constructed URL', () => {
        fc.assert(fc.property(fc.string({ minLength: 1, maxLength: 100 }), fc.string({ minLength: 1, maxLength: 40 }), (storedUrl, reference) => {
    ...
    it('property: fallback URL always contains the paynow_reference', () => {
        fc.assert(fc.property(fc.string({ minLength: 1, maxLength: 40 }).filter((s) => !s.includes('/')), (reference) => {
...
describe('Task 9.2: Paynow status parsing', () => {
    ...
    it('property: only paid maps to confirm', () => {
        fc.assert(fc.property(fc.string().filter((s) => s.toLowerCase() !== 'paid'), (status) => {
            expect(parsePaynowStatus(status)).not.toBe('confirm');
        }), { numRuns: 25 });
    });
});
//# sourceMappingURL=payment.webhook.test.js.map