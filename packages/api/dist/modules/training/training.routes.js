...
import { createTrainingEntry, listTrainingEntries, deleteTrainingEntry, updateWhatsAppProfile, isFileSizeValid, MAX_FILE_SIZE_BYTES, } from './training.service.js';
const VALID_TYPES = ['description', 'faq', 'tone_guidelines', 'logo', 'document'];
export async function trainingRoutes(app) {
    ...
    app.delete('/training/:id', { preHandler: authenticate }, async (request, reply) => {
        ...
        if (!deleted)
            return reply.status(404).send({ error: 'Training entry not found.' });
