...
import { sendVerificationEmail, sendPasswordResetEmail, sendLockoutEmail, } from '../../services/notification.stub.js';
...
export async function registerBusiness(data) {
    ...
    if (!validatePassword(password)) {
        throw new Error('Password must be at least 8 characters and contain an uppercase letter, a lowercase letter, and a digit.');
    }
    const passwordHash = await bcrypt.hash(password, BCRYPT_COST);
    let id;
    try {
        const result = await pool.query(`INSERT INTO businesses (name, owner_name, email, password_hash, email_verified)
       VALUES ($1, $2, $3, $4, TRUE)
       RETURNING id`, [businessName, ownerName, email.toLowerCase().trim(), passwordHash]);
        id = result.rows[0].id;
    }
    catch (err) {
...
async function sendVerificationEmail_internal(businessId, email) {
...
export async function verifyEmail(token) {
    ...
    await pool.query(`UPDATE businesses SET email_verified = TRUE, updated_at = NOW() WHERE id = $1`, [businessId]);
    await redis.del(`email_verify:${token}`);
    return businessId;
}
// ─── Task 2.5: Login Endpoint (Property 3) ───────────────────────────────────
export async function login(email, password) {
    const result = await pool.query(`SELECT id, password_hash, email_verified, failed_login_attempts, locked_until, status
...
export async function resetPassword(token, newPassword) {
    ...
    await pool.query(`UPDATE businesses SET password_hash = $1, updated_at = NOW() WHERE id = $2`, [passwordHash, businessId]);
