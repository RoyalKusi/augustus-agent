...
import { isValidTier } from '../subscription/plans.js';
...
function canDispatchAI(manualInterventionActive, budgetAllowed) {
...
function isValidReceipt(receipt) {
    return (typeof receipt.orderReference === 'string' &&
...
function orderStatusAfterPayment(currentStatus) {
...
describe('Task 17.1: Full Sales Conversation Flow', () => {
    ...
    it('AI dispatch allowed when no manual intervention and budget not exceeded', () => {
        fc.assert(fc.property(fc.boolean(), fc.boolean(), (manualActive, budgetAllowed) => {
    ...
    it('payment receipt contains all required fields: orderReference, items, total, timestamp', () => {
        fc.assert(fc.property(fc.record({
            ...
            items: fc.array(fc.record({ name: fc.string({ minLength: 1, maxLength: 20 }), qty: fc.integer({ min: 1, max: 10 }) }), { minLength: 1, maxLength: 5 }),
    ...
    it('stock decrement after payment: new stock = old stock - quantity', () => {
        fc.assert(fc.property(fc.integer({ min: 1, max: 1000 }), fc.integer({ min: 1, max: 100 }), (oldStock, quantity) => {
    ...
    it('order status transitions: pending → completed on payment confirmation', () => {
        fc.assert(fc.property(fc.constantFrom('pending', 'completed', 'expired', 'failed'), (status) => {
...
function activateInterventionPure(conv, agentId, now) {
    return {
        ...conv,
...
function deactivateInterventionPure(conv, now) {
    return {
        ...conv,
...
function canAgentSendMessage(conv) {
...
describe('Task 17.2: Manual Intervention Flow', () => {
    ...
    it('activating intervention blocks AI (isManualInterventionActive returns true)', () => {
        fc.assert(fc.property(fc.string({ minLength: 1, maxLength: 36 }), fc.date(), (agentId, now) => {
            const initial = {
                manual_intervention_active: false,
    ...
    it('deactivating intervention allows AI (isManualInterventionActive returns false)', () => {
        fc.assert(fc.property(fc.string({ minLength: 1, maxLength: 36 }), fc.date(), fc.date(), (agentId, startTime, endTime) => {
            const initial = {
                manual_intervention_active: false,
                intervention_start: null,
                intervention_end: null,
    ...
    it('after deactivation, intervention_end is set (non-null)', () => {
        fc.assert(fc.property(fc.string({ minLength: 1, maxLength: 36 }), fc.date({ min: new Date('2020-01-01'), max: new Date('2024-01-01') }), fc.date({ min: new Date('2024-01-02'), max: new Date('2030-01-01') }), (agentId, startTime, endTime) => {
            const initial = {
                manual_intervention_active: false,
                intervention_start: null,
                intervention_end: null,
            ...
            expect(deactivated.intervention_end.getTime()).toBeGreaterThanOrEqual(deactivated.intervention_start.getTime());
        }), { numRuns: 25 });
    });
    it('agent can send messages only when intervention is active', () => {
        fc.assert(fc.property(fc.boolean(), (active) => {
            const conv = {
...
const PLAN_TIERS = ['silver', 'gold', 'platinum'];
...
export function isPlanUpgrade(currentPlan, newPlan) {
    if (!isValidTier(currentPlan) || !isValidTier(newPlan))
        return false;
...
function canUpgrade(status, currentPlan, newPlan) {
    return status === 'active' && isPlanUpgrade(currentPlan, newPlan);
}
function canDowngrade(status, currentPlan, newPlan) {
    return status === 'active' && isPlanDowngrade(currentPlan, newPlan);
}
function canReactivate(status) {
...
describe('Task 17.3: Subscription Lifecycle', () => {
    ...
    it('isPlanUpgrade returns true only when newPlan is higher tier', () => {
        fc.assert(fc.property(fc.constantFrom(...PLAN_TIERS), fc.constantFrom(...PLAN_TIERS), (current, next) => {
    ...
    it('suspended subscription can be reactivated', () => {
        fc.assert(fc.property(fc.constantFrom('active', 'suspended', 'cancelled'), (status) => {
...
function checkBudgetStatus(accumulatedCostUsd, capUsd) {
    if (accumulatedCostUsd >= capUsd)
        return 'suspended';
    return 'active';
}
function checkBudgetAllowed(accumulatedCostUsd, capUsd) {
    return checkBudgetStatus(accumulatedCostUsd, capUsd) === 'active';
}
function isAlert80Triggered(accumulatedCostUsd, capUsd) {
    return capUsd > 0 && accumulatedCostUsd / capUsd >= 0.8;
}
function isAlert95Triggered(accumulatedCostUsd, capUsd) {
...
describe('Task 17.4: Budget Exhaustion Flow', () => {
    ...
    it('when cost >= cap, AI is suspended', () => {
        fc.assert(fc.property(fc.integer({ min: 1, max: 100000 }).map((n) => n / 100), fc.integer({ min: 1, max: 100000 }).map((n) => n / 100), (cap, extra) => {
    ...
    it('when cost < cap, AI is active', () => {
        fc.assert(fc.property(fc.integer({ min: 2, max: 100000 }).map((n) => n / 100), fc.integer({ min: 1, max: 100000 }).map((n) => n / 100), (cap, cost) => {
            fc.pre(cost < cap);
    ...
    it('after cycle reset (cost = 0), AI resumes', () => {
        fc.assert(fc.property(fc.integer({ min: 1, max: 100000 }).map((n) => n / 100), (cap) => {
    ...
    it('80% threshold alert: cost/cap >= 0.8 triggers alert', () => {
        fc.assert(fc.property(fc.integer({ min: 100, max: 10000 }).map((n) => n / 100), fc.integer({ min: 0, max: 100 }).map((n) => n / 100), (cap, ratio) => {
    ...
    it('80% alert does not trigger below threshold', () => {
        fc.assert(fc.property(fc.integer({ min: 100, max: 10000 }).map((n) => n / 100), fc.integer({ min: 0, max: 79 }).map((n) => n / 100), (cap, ratio) => {
    ...
    it('95% alert does not trigger below threshold', () => {
        fc.assert(fc.property(fc.integer({ min: 100, max: 10000 }).map((n) => n / 100), fc.integer({ min: 0, max: 94 }).map((n) => n / 100), (cap, ratio) => {
    ...
    it('shouldAutoProcess returns true when amount < threshold', () => {
        fc.assert(fc.property(fc.integer({ min: 1, max: 4999 }).map((n) => n / 100), (amount) => {
 ...
 */
export function isValidIntegration(wabaId, phoneNumberId, accessToken) {
    return (typeof wabaId === 'string' &&
...
export function reintegrationPreservesData(catalogueCount) {
...
describe('Task 17.5: WhatsApp Integration Setup and Re-integration', () => {
    ...
    it('valid integration has non-empty wabaId, phoneNumberId, accessToken', () => {
        fc.assert(fc.property(fc.string({ minLength: 1, maxLength: 64 }).filter((s) => s.trim().length > 0), fc.string({ minLength: 1, maxLength: 64 }).filter((s) => s.trim().length > 0), fc.string({ minLength: 1, maxLength: 128 }).filter((s) => s.trim().length > 0), (wabaId, phoneNumberId, accessToken) => {
    ...
    it('all three fields must be non-empty for a valid integration', () => {
        fc.assert(fc.property(fc.record({
            wabaId: fc.oneof(fc.constant(''), fc.string({ minLength: 1, maxLength: 32 }).filter((s) => s.trim().length > 0)),
            phoneNumberId: fc.oneof(fc.constant(''), fc.string({ minLength: 1, maxLength: 32 }).filter((s) => s.trim().length > 0)),
            accessToken: fc.oneof(fc.constant(''), fc.string({ minLength: 1, maxLength: 64 }).filter((s) => s.trim().length > 0)),
        }), ({ wabaId, phoneNumberId, accessToken }) => {
            const valid = isValidIntegration(wabaId, phoneNumberId, accessToken);
            const allNonEmpty = wabaId.trim().length > 0 &&
                phoneNumberId.trim().length > 0 &&
                accessToken.trim().length > 0;
    ...
    it('re-integration preserves existing catalogue data (catalogue is not cleared)', () => {
        fc.assert(fc.property(fc.integer({ min: 0, max: 10000 }), (catalogueCount) => {
    ...
    it('re-integration with any catalogue size always preserves data', () => {
        fc.assert(fc.property(fc.integer({ min: 0, max: 10000 }), fc.string({ minLength: 1, maxLength: 64 }).filter((s) => s.trim().length > 0), fc.string({ minLength: 1, maxLength: 64 }).filter((s) => s.trim().length > 0), fc.string({ minLength: 1, maxLength: 128 }).filter((s) => s.trim().length > 0), (catalogueCount, wabaId, phoneNumberId, accessToken) => {
...
//# sourceMappingURL=e2e.properties.test.js.map