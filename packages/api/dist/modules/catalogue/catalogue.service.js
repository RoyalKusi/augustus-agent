...
export async function createProduct(businessId, data) {
    const result = await pool.query(`INSERT INTO products (business_id, name, description, price, currency, stock_quantity, category, image_urls)
...
export async function getProduct(businessId, productId) {
    const result = await pool.query(`SELECT * FROM products WHERE id = $1 AND business_id = $2`, [productId, businessId]);
...
export async function updateProduct(businessId, productId, data) {
    ...
    const result = await pool.query(`UPDATE products SET ${fields.join(', ')} WHERE id = $${idx++} AND business_id = $${idx} RETURNING *`, values);
    return result.rows.length > 0 ? rowToProduct(result.rows[0]) : null;
}
export async function deleteProduct(businessId, productId) {
    const result = await pool.query(`DELETE FROM products WHERE id = $1 AND business_id = $2`, [productId, businessId]);
 ...
 */
export async function getInStockProducts(businessId) {
 ...
 */
export async function searchProducts(businessId, filters) {
    ...
    const result = await pool.query(`SELECT * FROM products WHERE ${conditions.join(' AND ')} ORDER BY name`, values);
    return result.rows.map(rowToProduct);
}
// ─── Task 8.4: Promotional Combos ────────────────────────────────────────────
export async function createCombo(businessId, data) {
    ...
    try {
        ...
        const comboResult = await client.query(`INSERT INTO promo_combos (business_id, name, promo_price, currency)
       VALUES ($1, $2, $3, $4) RETURNING *`, [businessId, data.name, data.promoPrice, data.currency]);
        const combo = comboResult.rows[0];
        for (const productId of data.productIds) {
            await client.query(`INSERT INTO promo_combo_products (combo_id, product_id) VALUES ($1, $2)`, [combo.id, productId]);
        }
...
export async function listCombos(businessId) {
    const combosResult = await pool.query(`SELECT * FROM promo_combos WHERE business_id = $1 ORDER BY created_at DESC`, [businessId]);
    const combos = [];
    for (const row of combosResult.rows) {
        const productsResult = await pool.query(`SELECT product_id FROM promo_combo_products WHERE combo_id = $1`, [row.id]);
        combos.push({
            id: row.id,
            businessId: row.business_id,
            name: row.name,
...
export async function updateCombo(businessId, comboId, data) {
    const fields = [];
    const values = [];
    let idx = 1;
    if (data.name !== undefined) {
    ...
    try {
        ...
        if (fields.length > 0) {
            ...
            const result = await client.query(`UPDATE promo_combos SET ${fields.join(', ')} WHERE id = $${idx++} AND business_id = $${idx} RETURNING id`, values);
        ...
        if (data.productIds !== undefined) {
            ...
            for (const productId of data.productIds) {
                await client.query(`INSERT INTO promo_combo_products (combo_id, product_id) VALUES ($1, $2)`, [comboId, productId]);
            }
        }
        await client.query('COMMIT');
    }
    ...
    const comboResult = await pool.query(`SELECT * FROM promo_combos WHERE id = $1 AND business_id = $2`, [comboId, businessId]);
    if (comboResult.rows.length === 0)
        return null;
    const row = comboResult.rows[0];
    const productsResult = await pool.query(`SELECT product_id FROM promo_combo_products WHERE combo_id = $1`, [comboId]);
    return {
        id: row.id,
        businessId: row.business_id,
        name: row.name,
...
export async function deleteCombo(businessId, comboId) {
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        await client.query(`DELETE FROM promo_combo_products WHERE combo_id = $1`, [comboId]);
        const result = await client.query(`DELETE FROM promo_combos WHERE id = $1 AND business_id = $2`, [comboId, businessId]);
        await client.query('COMMIT');
        return (result.rowCount ?? 0) > 0;
    }
    catch (err) {
 ...
 */
export async function getActiveComboCarouselItems(businessId) {
    const result = await pool.query(`SELECT id, name, promo_price, currency FROM promo_combos
...
const REQUIRED_CSV_FIELDS = ['name', 'price', 'currency', 'stock_quantity'];
/**
 ...
 */
export async function importProductsFromCsv(businessId, csvText) {
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        await pool.query(`INSERT INTO products (business_id, name, description, price, currency, stock_quantity, category)
 ...
 */
export async function getProductRevenue(businessId, productId) {
    const result = await pool.query(`SELECT
     ...
     WHERE oi.product_id = $1
       AND o.business_id = $2
       AND o.status = 'completed'`, [productId, businessId]);
