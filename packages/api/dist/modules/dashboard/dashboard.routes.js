...
import { getSubscriptionOverview, getCreditUsage, getActiveConversations, getOrdersSummary, getRevenueSummary, getOrdersCsv, getWithdrawalHistory, createSupportTicket, listSupportTickets, updateSupportTicketStatus, updateOrderStatus, } from './dashboard.service.js';
export async function dashboardRoutes(app) {
    ...
    app.get('/dashboard/conversations/:id/messages', { preHandler: authenticate }, async (request, reply) => {
        ...
        try {
            const result = await pool.query(`SELECT id, direction, content, created_at
         FROM messages
         WHERE conversation_id = $1 AND business_id = $2
         ORDER BY created_at ASC
         LIMIT 100`, [id, request.businessId]);
    ...
    app.post('/dashboard/support', { preHandler: authenticate }, async (request, reply) => {
        ...
        try {
            const ticket = await createSupportTicket(request.businessId, body.subject, body.description, body.attachmentUrl);
    ...
    app.patch('/dashboard/support/:id/status', { preHandler: authenticate }, async (request, reply) => {
        const { id } = request.params;
        const body = request.body;
        if (!body.status) {
            return reply.status(400).send({ error: 'Missing required field: status.' });
