...
vi.mock('../../whatsapp/message-dispatcher.js', () => ({ sendMessage: vi.fn() }));
...
import { filterContextWindow, isManualInterventionActive, buildSystemPrompt, detectLanguage, parseClaudeResponse, isSessionExpired, CLAUDE_HAIKU_MODEL, MAX_CONTEXT_MESSAGES, CONTEXT_WINDOW_MS, processInboundMessage, } from '../conversation-engine.service.js';
afterEach(() => { vi.restoreAllMocks(); vi.clearAllMocks(); });
const strArb = fc.string({ minLength: 1, maxLength: 64 }).filter((s) => !s.includes('\0'));
const uuidArb = fc.uuid();
const msgArb = (nowMs) => fc.record({ role: fc.constantFrom('user', 'assistant'), content: strArb, ageMs: fc.integer({ min: 0, max: 7200000 }) })
    .map(({ role, content, ageMs }) => ({ role, content, timestamp: nowMs - ageMs }));
const msgListArb = (nowMs) => fc.array(msgArb(nowMs), { minLength: 0, maxLength: 50 });
const convRowArb = (manual) => fc.record({ id: uuidArb, business_id: uuidArb, customer_wa_number: strArb, message_count: fc.integer({ min: 0, max: 100 }) })
    .map((r) => ({ ...r, session_start: new Date(Date.now() - 30 * 60 * 1000), session_end: null, manual_intervention_active: manual, manual_agent_id: null, intervention_start: null, intervention_end: null, context_summary: null, status: 'active' }));
function makeConv(manual, overrides = {}) {
...
// Feature: augustus-ai-sales-platform, Property 13: Claude Haiku Is the Only Inference Model
// **Validates: Requirements 5.3**
describe('Property 13: Claude Haiku Is the Only Inference Model', () => {
    it('CLAUDE_HAIKU_MODEL always contains haiku', () => { expect(CLAUDE_HAIKU_MODEL.toLowerCase()).toContain('haiku'); });
    it('model parameter sent to Claude API always equals CLAUDE_HAIKU_MODEL', async () => {
        await fc.assert(fc.asyncProperty(strArb, strArb, async (sp, um) => {
            let body = null;
            vi.spyOn(globalThis, 'fetch').mockImplementationOnce(async (_u, init) => { body = JSON.parse(init?.body ?? '{}'); return new Response(JSON.stringify({ content: [{ type: 'text', text: 'Hi' }], usage: { input_tokens: 10, output_tokens: 5 } }), { status: 200 }); });
            const { callClaudeHaiku } = await import('../conversation-engine.service.js');
            await callClaudeHaiku(sp, [], um);
            expect(body).not.toBeNull();
            expect(body['model']).toBe(CLAUDE_HAIKU_MODEL);
            expect(body['model'].toLowerCase()).toContain('haiku');
        }), { numRuns: 25 });
    });
    it('model is never sonnet, opus, or gpt', async () => {
        await fc.assert(fc.asyncProperty(strArb, strArb, async (sp, um) => {
            let m = null;
            vi.spyOn(globalThis, 'fetch').mockImplementationOnce(async (_u, init) => { m = JSON.parse(init?.body ?? '{}')['model']; return new Response(JSON.stringify({ content: [{ type: 'text', text: 'ok' }], usage: { input_tokens: 5, output_tokens: 3 } }), { status: 200 }); });
            const { callClaudeHaiku } = await import('../conversation-engine.service.js');
            await callClaudeHaiku(sp, [], um);
            expect(m).not.toBeNull();
            expect(m.toLowerCase()).not.toContain('sonnet');
            expect(m.toLowerCase()).not.toContain('opus');
            expect(m.toLowerCase()).not.toContain('gpt');
...
describe('Property 14: Session Context Window Boundary', () => {
    ...
    it('filtered context never contains messages older than 60 minutes', () => { const n = Date.now(); fc.assert(fc.property(msgListArb(n), (msgs) => { const r = filterContextWindow(msgs, n); const c = n - CONTEXT_WINDOW_MS; for (const m of r)
        expect(m.timestamp).toBeGreaterThanOrEqual(c); }), { numRuns: 25 }); });
    it('result satisfies both constraints simultaneously', () => { const n = Date.now(); fc.assert(fc.property(msgListArb(n), (msgs) => { const r = filterContextWindow(msgs, n); const c = n - CONTEXT_WINDOW_MS; expect(r.length).toBeLessThanOrEqual(MAX_CONTEXT_MESSAGES); for (const m of r)
        expect(m.timestamp).toBeGreaterThanOrEqual(c); }), { numRuns: 25 }); });
    it('message exactly at 60-minute boundary is included', () => { const n = Date.now(); expect(filterContextWindow([{ role: 'user', content: 'b', timestamp: n - CONTEXT_WINDOW_MS }], n).length).toBe(1); });
    it('message 1ms past 60-minute boundary is excluded', () => { const n = Date.now(); expect(filterContextWindow([{ role: 'user', content: 'o', timestamp: n - CONTEXT_WINDOW_MS - 1 }], n).length).toBe(0); });
    it('when 40 messages in window, only most recent 30 returned', () => { const n = Date.now(); const msgs = Array.from({ length: 40 }, (_, i) => ({ role: 'user', content: 'x', timestamp: n - (39 - i) * 1000 })); const r = filterContextWindow(msgs, n); expect(r.length).toBe(MAX_CONTEXT_MESSAGES); const expected30 = msgs.slice(10); expect(r.map((m) => m.timestamp)).toEqual(expected30.map((m) => m.timestamp)); });
...
describe('Property 15: Manual Intervention Blocks AI Responses', () => {
    ...
    it('processInboundMessage dispatches no AI message when manual_intervention_active = true', async () => {
        await fc.assert(fc.asyncProperty(uuidArb, strArb, strArb, async (bid, cwa, mt) => {
            ...
            vi.mocked(pool.query).mockResolvedValueOnce({ rows: [makeConv(true, { id: 'c1', business_id: bid, customer_wa_number: cwa })], rowCount: 1 });
            vi.mocked(sendMessage).mockReset();
            const r = await processInboundMessage({ businessId: bid, customerWaNumber: cwa, messageText: mt, messageId: 'm1', timestamp: Date.now() });
            expect(r.dispatched).toBe(false);
            expect(r.skippedManualIntervention).toBe(true);
    ...
    it('no Claude API fetch when manual intervention active', async () => {
        await fc.assert(fc.asyncProperty(uuidArb, strArb, strArb, async (bid, cwa, mt) => {
            vi.mocked(pool.query).mockReset();
            vi.mocked(pool.query).mockResolvedValueOnce({ rows: [makeConv(true, { id: 'c2', business_id: bid, customer_wa_number: cwa })], rowCount: 1 });
            const spy = vi.spyOn(globalThis, 'fetch');
            await processInboundMessage({ businessId: bid, customerWaNumber: cwa, messageText: mt, messageId: 'm2', timestamp: Date.now() });
            expect(spy.mock.calls.filter((c) => typeof c[0] === 'string' && c[0].includes('anthropic.com')).length).toBe(0);
        }), { numRuns: 25 });
    });
});
...
describe('Property 16: Deactivating Manual Intervention Restores AI Responses', () => {
    ...
    it('transition active->inactive enables AI processing', () => {
        fc.assert(fc.property(fc.record({ id: uuidArb, business_id: uuidArb, customer_wa_number: strArb, message_count: fc.integer({ min: 0, max: 10 }) }), (base) => {
            const a = { ...base, session_start: new Date(), session_end: null, manual_intervention_active: true, manual_agent_id: 'a1', intervention_start: new Date(), intervention_end: null, context_summary: null, status: 'active' };
            const d = { ...a, manual_intervention_active: false, intervention_end: new Date() };
            expect(isManualInterventionActive(a)).toBe(true);
            expect(isManualInterventionActive(d)).toBe(false);
        }), { numRuns: 25 });
    });
    it('processInboundMessage proceeds to AI when manual_intervention_active = false', async () => {
        await fc.assert(fc.asyncProperty(uuidArb, strArb, strArb, async (bid, cwa, mt) => {
            const mq = vi.mocked(pool.query);
            mq.mockReset();
            mq.mockResolvedValueOnce({ rows: [makeConv(false, { id: 'c3', business_id: bid, customer_wa_number: cwa, message_count: 2 })], rowCount: 1 });
            ...
            mq.mockResolvedValueOnce({ rows: [], rowCount: 0 });
            mq.mockResolvedValueOnce({ rows: [], rowCount: 0 });
            mq.mockResolvedValueOnce({ rows: [{ context_summary: null }], rowCount: 1 });
            ...
            mq.mockResolvedValueOnce({ rows: [{ email: 'b@e.com' }], rowCount: 1 });
            ...
            mq.mockResolvedValueOnce({ rows: [], rowCount: 1 });
            mq.mockResolvedValueOnce({ rows: [], rowCount: 1 });
            mq.mockResolvedValueOnce({ rows: [], rowCount: 1 });
            vi.mocked(redisConv.appendMessage).mockResolvedValue(undefined);
            const r = await processInboundMessage({ businessId: bid, customerWaNumber: cwa, messageText: mt, messageId: 'm3', timestamp: Date.now() });
            expect(r.dispatched).toBe(true);
            expect(r.skippedManualIntervention).toBeUndefined();
            expect(sendMessage).toHaveBeenCalled();
        }), { numRuns: 25 });
    });
});
describe('buildSystemPrompt', () => {
    it('always includes non-disclosure instruction', () => { fc.assert(fc.property(fc.constantFrom('English', 'French', 'Spanish'), fc.option(strArb, { nil: null }), (lang, cs) => { const p = buildSystemPrompt(null, [], lang, cs); expect(p.toLowerCase()).toContain('never reveal'); expect(p.toLowerCase()).toContain('system prompt'); }), { numRuns: 25 }); });
    it('always includes language instruction', () => { fc.assert(fc.property(fc.constantFrom('English', 'French', 'Spanish', 'Arabic', 'Chinese'), (lang) => { const p = buildSystemPrompt(null, [], lang, null); expect(p).toContain(lang); expect(p.toLowerCase()).toContain('respond only in'); }), { numRuns: 25 }); });
});
...
//# sourceMappingURL=conversation.properties.test.js.map