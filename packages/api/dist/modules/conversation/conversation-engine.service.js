...
export function buildSystemPrompt(trainingData, products, detectedLanguage, contextSummary, inChatPaymentsEnabled = true) {
    ...
    if (trainingData) {
        if (trainingData.business_description)
            parts.push('## Business Description\n' + trainingData.business_description);
        if (trainingData.faqs)
            parts.push('## FAQs\n' + trainingData.faqs);
        if (trainingData.tone_guidelines)
...
export function parseClaudeResponse(responseText) {
    ...
    if (paymentMatch) {
        let orderDetails = {};
        try {
            orderDetails = JSON.parse(paymentMatch[1]);
        }
        catch {
...
export async function processInboundMessage(msg) {
    ...
    if (!budgetAllowed) {
        ...
        if (shouldNotify)
            await sendMessage(businessId, { type: 'text', to: customerWaNumber, body: 'Our AI assistant is temporarily unavailable. Please try again later.' });
    ...
    const [trainingData, products, settingsResult] = await Promise.all([
        ...
        pool.query('SELECT in_chat_payments_enabled, external_payment_details FROM businesses WHERE id = $1', [businessId]),
    ]);
    ...
    if (action.type === 'carousel' && action.products?.length > 0) {
        ...
        const productRows = await pool.query('SELECT id, name, price, currency, image_urls FROM products WHERE id = ANY($1) AND business_id = $2 AND is_active = TRUE', [action.products, businessId]);
            ...
            if (items.length > 0) {
                if (settings?.in_chat_payments_enabled) {
                    ...
                    for (const item of items) {
                        const prodResult = await pool.query('SELECT name, price, currency FROM products WHERE id = $1 AND business_id = $2', [item.product_id, businessId]);
...
export function stopConversationEngineConsumer() {
