  ...
  return (
    <div style={{ maxWidth: 800 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        ...
        <button
          onClick={load}
          style={{ padding: '6px 14px', background: '#edf2f7', border: '1px solid #cbd5e0', borderRadius: 6, cursor: 'pointer', fontSize: 13 }}
        >
          ↻ Refresh
        </button>
      </div>
        ...
        <div key={conv.id} style={cardStyle(conv.manualInterventionActive)}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
              ...
              <button
                onClick={() => toggleIntervention(conv)}
                style={conv.manualInterventionActive ? successBtn : dangerBtn}
              >
          ...
          {/* Expanded: message history + send box */}
          {expanded[conv.id] && (
            <div style={{ marginTop: 12 }}>
              {/* Message thread */}
              <div style={threadStyle}>
                {(convMessages[conv.id] ?? []).length === 0 ? (
                  <p style={{ color: '#a0aec0', fontSize: 13, textAlign: 'center', margin: '12px 0' }}>
              ...
              {/* Send message — only when intervention is active */}
              {conv.manualInterventionActive ? (
                <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                  ...
                  <button
                    ...
                    disabled={sending[conv.id] || !messages[conv.id]?.trim()}
                    style={primaryBtn}
                  >
